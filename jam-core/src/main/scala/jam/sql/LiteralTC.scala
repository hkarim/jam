...
trait LiteralTC[DBF[_], R[_], W[_]] { self: Backend[DBF, R, W] =>
  ...
  trait Literal[A] { l =>
    ...
    def contramap[B](f: B => A): Literal[B] = (instance: B) => l.fragment(f(instance))
  }
  ...
  trait LiteralLowPriorityInstances extends LiteralLowerPriorityInstances {
    ...
    implicit def numeric[N: Numeric]: Literal[N] = (a: N) => Vector(const(s"$a"))
  ...
  object Literal extends ProductTypeClassCompanion[Literal] with LiteralLowPriorityInstances {
    ...
    @inline implicit def deriveLiteralHCons[H, T <: HList](implicit ch: Lazy[Literal[H]],
  ...
  implicit class LiteralOps[A](a: A) {
    def literal(implicit ev: Literal[A]): LiteralNode[A] = LiteralNode(ev.fragment(a))
  }

  implicit def literalToConstant[A](implicit l: Literal[A]): Constant[A] = (a: A) => LiteralNode[A](l.fragment(a))
