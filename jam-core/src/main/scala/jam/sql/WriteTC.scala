...
trait WriteTC[DBF[_], R[_], W[_]] { self: Backend[DBF, R, W] =>
  ...
  object Write
      extends ProductTypeClassCompanion[Write]
      with WriteLowPriorityInstances {
    ...
    @inline implicit def deriveWriteHCons[H, T <: HList](
        implicit ch: Lazy[Write[H]],
        ct: Lazy[Write[T]]): Write[H :: T] =
      typeClass.product(ch.value, ct.value)

    @inline implicit def deriveWriteInstance[F, G](
        implicit gen: Generic.Aux[F, G],
        cg: Lazy[Write[G]]): Write[F] =
