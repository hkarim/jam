...
trait SlickSyntax { self: Slick =>
  ...
  implicit def entitySyntax[E](
      implicit ec: ExecutionContext): EntitySyntax[E] = new EntitySyntax[E] {
    def findOne(e: Entity[E])(expression: Expression[Boolean])(
        implicit r: Read[E],
        ns: NamingStrategy): DBIO[Option[E]] =
      self
    ...
    def find(e: Entity[E])(expression: Expression[Boolean])(
        implicit r: Read[E],
        ns: NamingStrategy): DBIO[Vector[E]] =
  ...
  implicit def dqlSyntax[A](implicit ec: ExecutionContext): DQLSyntax[A] =
    new DQLSyntax[A] {
      def query(n: DQLNode[A])(implicit r: Read[A],
                               ns: NamingStrategy): DBIO[Vector[A]] =
        self.query[A](n)

      def one(n: DQLNode[A])(implicit r: Read[A],
                             ns: NamingStrategy): DBIO[Option[A]] =
        self.query[A](n).map(_.headOption)
    }

  implicit def dqlConversionSyntax[L <: HList](
      implicit ec: ExecutionContext): DQLConversionSyntax[L] =
